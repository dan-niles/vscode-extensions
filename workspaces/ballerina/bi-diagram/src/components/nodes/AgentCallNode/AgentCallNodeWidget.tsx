...
const syncPulseAnimationDark = keyframes`
    0% { filter: drop-shadow(0 0 2px color-mix(in srgb, ${CHART_COLORS.BRIGHT_CYAN} 30%, transparent)); }
    100% { filter: drop-shadow(0 0 8px color-mix(in srgb, ${CHART_COLORS.BRIGHT_CYAN} 60%, transparent)) drop-shadow(0 0 12px color-mix(in srgb, ${CHART_COLORS.BRIGHT_CYAN} 30%, transparent)); }
`;

const syncPulseAnimationLight = keyframes`
    0% { filter: drop-shadow(0 0 2px color-mix(in srgb, ${CHART_COLORS.CYAN} 30%, transparent)); }
    100% { filter: drop-shadow(0 0 8px color-mix(in srgb, ${CHART_COLORS.CYAN} 60%, transparent)) drop-shadow(0 0 12px color-mix(in srgb, ${CHART_COLORS.CYAN} 30%, transparent)); }
`;

const boxSyncPulseAnimationDark = keyframes`
    0% { box-shadow: 0 0 3px color-mix(in srgb, ${CHART_COLORS.BRIGHT_CYAN} 20%, transparent); }
    100% { box-shadow: 0 0 10px color-mix(in srgb, ${CHART_COLORS.BRIGHT_CYAN} 50%, transparent), 0 0 20px color-mix(in srgb, ${CHART_COLORS.BRIGHT_CYAN} 20%, transparent); }
`;

const boxSyncPulseAnimationLight = keyframes`
    0% { box-shadow: 0 0 3px color-mix(in srgb, ${CHART_COLORS.CYAN} 20%, transparent); }
    100% { box-shadow: 0 0 10px color-mix(in srgb, ${CHART_COLORS.CYAN} 50%, transparent), 0 0 20px color-mix(in srgb, ${CHART_COLORS.CYAN} 20%, transparent); }
`;
...
export function AgentCallNodeWidget(props: AgentCallNodeWidgetProps) {
    ...
    const syncPulseAnimation = isDarkMode ? syncPulseAnimationDark : syncPulseAnimationLight;
    const boxSyncPulseAnimation = isDarkMode ? boxSyncPulseAnimationDark : boxSyncPulseAnimationLight;
    ...
    const isTraceMatch = traceAnimation && (() => {
        ...
        if (sysInstr) {
            ...
            return (nodeRole && sysInstr.includes(nodeRole)) ||
                (nodeInstructions && sysInstr.includes(nodeInstructions));
        }
    ...
    return (
        <NodeStyles.Node data-testid="agent-call-node" readOnly={readOnly}>
            ...
            >
                {/* Overlay for smooth Agent Box pulsing transition */}
                <div
                    css={css`
                        position: absolute;
